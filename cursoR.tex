...
\begin{verbatim}
##  [1] "segunda" "segunda" "segunda" "segunda" "segunda" "segunda" "segunda"
...
##  [1] 17686.16 17686.20 17686.24 17686.28 17686.32 17686.37 17686.41
##  [8] 17686.45 17686.49 17686.53 17686.57 17686.62 17686.66 17686.70
## [15] 17686.74 17686.78 17686.82 17686.87 17686.91 17686.95 17686.99
## [22] 17687.03 17687.07 17687.12
## attr(,"origin")
## [1] "1970-01-01 GMT"
\end{verbatim}
...
\begin{verbatim}
##   [1] segunda segunda segunda segunda segunda segunda segunda segunda
...
Vamos ler alguns dados usando read.table, para saber o que a função faz,
use \texttt{?read.table}. Os argumentos da função são:
...
O terceiro argumento é sep, com valores por default = ``''. Esse
argumento indica para o R qual o tipo de separador utilizado entre as
colunas dos dados.
...
Lembre-se que as funções head e tail permitem que você veja as primeiras
ou últimas linhas do data frame.
...
O que acontece se tentarmos ler os mesmo dados utilizando a função
read.table?
...
utilizado, como o segundo arquivo tem separação de ``;'', precisamos
...
do R, vamos exportá-lo de uma forma que ele possa ser lido em algum
Excel genérico. Não esqueça de usar o que foi ensinado acima.
...
Muito cuidado ao utilizar esse comando, pois podemos cometer alguns
delizes, como trocar o nome do objeto. Veja esse exemplo abaixo:
...
Existem inúmeros pacotes para organizar (Arrange) e modificar data
frames. As mais conhecidas são provavelmente do universe
\emph{tidyverse} como o famoso pacote \emph{dplyr}. Mas, neste curso
vamos focar no \textbf{base}.

Vamos então revisar a classe de cada coluna do nosso data-frame com a
função \texttt{sapply}, que será revisada em putro capítuo, mas lembre,
...
``POSIXct'', porém, a calsse do tipo Data é ``factor'' assim como a
Hora, o que pode ser ruim. Então, vamos criar uma variável de tempo mais
padronizada com o formato 2018-06-04 00:46:31.

Para isso temos que juntar as variáveis Data e Hora. Faremos isso numa
nova varável chamada ``tempo\_char''``, adicionando-a diretamente no
data frame com o cifrão DOLLAR''\$``. Podemos fazer isso com as funções
...
Para isso usamos a função \texttt{fread}.
...
\KeywordTok{seq}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{10}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{order}\NormalTok{(., }\DataTypeTok{decreasing =}\NormalTok{ T) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{paste}\NormalTok{(., }\StringTok{"vezes"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{wrf <-}\StringTok{ }\NormalTok{ncdf4}\OperatorTok{::}\KeywordTok{nc_open}\NormalTok{(}\StringTok{"dados/met_em.d03.2016-01-10.nc"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{wrf <-}\StringTok{ }\KeywordTok{nc_open}\NormalTok{(}\StringTok{"dados/met_em.d03.2016-01-10.nc"}\NormalTok{, }\DataTypeTok{write =}\NormalTok{ T)}
...
\includegraphics{cursoR_files/figure-latex/unnamed-chunk-88-1.pdf}
...
\begin{verbatim}
## Colour gradient: imagej_cmy, number: 3489
\end{verbatim}
